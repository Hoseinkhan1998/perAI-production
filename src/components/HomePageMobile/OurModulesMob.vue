...
<template>
  <div class="grid gap-5 grid-cols-12 px-5">
    ...
    <div class="col-span-full">
      <div class="flex flex-wrap justify-center gap-5">
        <div @click="navigateToModule(card)" v-for="(card, index) in cards" :key="card.id" class="skill-card" :class="{ visible: cardVisibility[index] }">
          <div class="flex flex-col justify-center cursor-pointer items-center">
            ...
            <h3 class="mt-2 text-center max-w-[12vh] font-semibold">{{ card.title }}</h3>
          </div>
        </div>
        <router-link
          to="/our-modules-cards"
          class="cursor-pointer max-w-[10vh] flex items-center gap-3 justify-center skill-card"
          :class="{ visible: cardVisibility[cards.length - 1] }">
          <p class="font-semibold text-lg">اطلاعات بیشتر</p>
